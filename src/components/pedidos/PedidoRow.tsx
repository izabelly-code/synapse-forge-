...
const STATUS_SEQUENCE: PedidoStatus[] = [
    "MODELAGEM",
    "IMPRESSAO",
    "PINTURA",
    "ACABAMENTO",
...
function tempoRelativo(
    ...
    const dias = Math.round(
        (hoje.getTime() - dia.getTime()) / 86_400_000
    );

    if (dias <= 0) {
        return t("pedidos.row.relToday");
    }

    if (dias === 1) {
        return t("pedidos.row.relYesterday");
    }

    if (dias < 7) {
        return t("pedidos.row.relDays", {
            count: dias
        });
    }

    if (dias < 30) {
        return t("pedidos.row.relWeeks", {
            count: Math.floor(dias / 7)
        });
    }

    if (dias < 365) {
        return t("pedidos.row.relMonths", {
            count: Math.floor(dias / 30)
        });
    }

    return t("pedidos.row.relYears", {
        count: Math.floor(dias / 365)
    });
}
...
function tempoRestante(
    ...
    if (diff < 0) {
        ...
        if (over >= D) {
            return {
                texto: t("pedidos.row.lateDays", {
                    count: Math.floor(over / D)
                }),
                tom: "atrasado"
            };
        }
        ...
        return {
            texto: t("pedidos.row.lateHours", {
                count: Math.max(1, Math.floor(over / H))
            }),
            tom: "atrasado"
        };
    }
    ...
    if (diff >= H) {
        ...
        return {
            texto: t("pedidos.row.hoursLeft", {
                count: Math.floor(diff / H)
            }),
            tom: "urgente"
        };
    }
    ...
    return {
        texto: t("pedidos.row.minutesLeft", {
            count: Math.max(
                1,
                Math.floor(diff / MIN)
            )
...
function PedidoRow({
    ...
    const naPrimeiraEtapa =
        pedido.status === "MODELAGEM";
    ...
    return (
        ...
        >
            ...
            <div className="cell cell-cliente">
                ...
                <span className="row-cliente-nome">
                    {pedido.cliente}
                </span>
            ...
            <div className="cell cell-projeto">
                ...
                {pedido.descricao && (
                    <span className="row-projeto-desc">
                        {pedido.descricao}
                    </span>
                )}
                ...
                {finalizado ? (
                    <>
                        ...
                        <span className="row-prazo row-prazo-done">
                            ...
                            {t(
                                "pedidos.status.FINALIZADO"
                            )}

                        </span>

                        <span className="row-prazo-sub">
                            {tempoRelativo(
                                pedido.atualizadoEm,
                                t
                            )}
                            ...
                            >
                                ...
                                {!confirmDel ? (
                                    ...
                                    <>
                                        ...
                                        <span className="kebab-confirm-text">
                                            {t(
                                                "pedidos.row.confirmDelete"
                                            )}
                                        </span>
                                        ...
                                        <div className="kebab-confirm-actions">
                                            ...
                                            <button
                                                className="kebab-confirm-cancel"
                                                onClick={() =>
                                                    setConfirmDel(
                                                        false
                                                    )
